...
void create_clients(game_t *game)
{
    ...
    sfTexture *texture = sfTexture_createFromFile("assets/imgs/perso4.png", 0);

    game->network->client_sprite = sfSprite_create(texture);
    sfSprite_setScale(game->network->client_sprite, (sfVector2f) {4, 4});
    sfSprite_setTexture(game->network->client_sprite, texture, 0);
    sfSprite_setTextureRect(game->network->client_sprite,
    (sfIntRect) {.position = {0, 0}, .size = {70, 85}});
    ...
    for (int i = 0; i < 100; i++) {
        ...
        (sfIntRect) {.position = {0, 0}, .size = {18, 18}};
    }
}
