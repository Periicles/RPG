...
static sfText *create_text(char *txt, const sfVector2f *pos, sfFont *font,
    int size)
{
...
static void create_background(game_t *game, game_menu_t *game_menu)
{
    ...
    sfRectangleShape_setSize(game_menu->container,
        (sfVector2f){game->params->window_size.x,
            game->params->window_size.y});
    ...
    sfRectangleShape_setSize(game_menu->content,
        (sfVector2f){game->params->window_size.x / 1.5,
            game->params->window_size.y / 1.5});
    sfRectangleShape_setPosition(game_menu->content,
        (sfVector2f){game->params->window_size.x / 6.5,
            game->params->window_size.y / 6.5});
...
void create_menu(game_t *game)
{
    ...
    pos = (sfVector2f){game->params->window_size.x / 2.8,
        game->params->window_size.y / 5};
    for (i = 0; title[i] != NULL; i++)
        game->game_menu->title[i] = create_text(title[i], &pos,
            game->game_menu->font, 50);
    create_sidebar(game);
