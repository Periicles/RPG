...
static void create_text(char *name, char *value, game_t *game, int i)
{
    ...
    sfText_setFillColor(name_text[i], sfWhite);
    sfText_setFillColor(value_text[i], sfWhite);
...
static void alloc_character(game_t *game)
{
...
{
    char *names[6] = {"NAME", "ATTACK", "DEFENSE", "STRENGTH", "SPEED",
        "LEVEL"};
    char *values[6] = {game->perso->name,
        my_itoa(game->perso->combat->attack),
    ...
    alloc_character(game);
    for (i = 0; i < 6; i++) {
        game->game_menu->character->name[i] =
            sfText_create(game->game_menu->font);
        game->game_menu->character->value[i] =
            sfText_create(game->game_menu->font);
