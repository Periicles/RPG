...
static void create_size_buttons(game_t *game, char *size[])
{
    ...
    for (i = 0; i < 2; i++) {
        ...
        sfText_setPosition(button[i]->text,
            (sfVector2f){button[i]->pos.x + 15, button[i]->pos.y + 14});
    }
...
{
    char *size[2] = {"Fullscren", "Windowed"};
    char *resolution[3] = {"1920x1080", "1280x720", "800x600"};

    game->params->visu->windows = malloc(sizeof(windows_t));
    game->params->visu->windows->size = malloc(sizeof(buttons_t *) * 2);
    game->params->visu->windows->resolution =
        malloc(sizeof(buttons_t *) * 3);
