...
void create_button(buttons_t *button, char *text, int i, game_t *game)
{
    ...
    button->pos = (sfVector2f){
        game->params->window_size.x / 10 + (i * 200), 20};
    ...
    sfText_setPosition(button->text,
        (sfVector2f){button->pos.x + 15, button->pos.y + 15});
    sfText_setString(button->text, text);
}

static void create_navbar_buttons(game_t *game)
{
    char *text[5] = {"FPS", "Music", "Keybinds", "Save", "Window"};
    int i = 0;
